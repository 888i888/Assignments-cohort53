...
export function tellFortune(numKids, partnerNames, locations, jobTitles) {
  ...
  const title = selectRandomly(jobTitles);

  return `You will be a ${title} in ${location}, married to ${name} with ${kids} kids.`;
}

function main() {
  const numKids = [1, 2, 3, 4, 5];

  const partnerNames = ['Igor', 'Vira', 'Marta', 'Julius', 'Luka'];
